...
(defn load-json-cache [name]
  (go-loop [suffixes [".cljs" ".cljc" ""]
            res nil]
    (if (seq suffixes)
      (let [suffix (first suffixes)
            cache (<! (http/get (filename-of (str name suffix ".cache.json") (cache-buster?)) {:with-credentials? false}))]
        (if (= 200 (:status cache))
          cache
          (recur (rest suffixes) cache)))
      res)))
...
(defn load-ns-from-cache [name src-cb macro?]
  ...
  (go
    (let [nn (name->cached-resource name)
          ...
          cache (<! (load-json-cache (str root nn suffix)))]
...
(defn cached-macro-ns-regexp []
  (:clojure_cached_macro_ns_regexp (klipse-settings) #"cljs\.reader|cljs\.core\.[async|match].*|clojure\.math\.macros|gadjett\.core|cljs\.test|clojure\.test|clojure.test.check.*|reagent\..*|om\..*|cljs\.spec.*|cljs-time\..*|re-frame\..*|net\.cgrand\.macrovich|reagent-forms\..*|ajax\.macros|poppea|cljs\.tools\.reader\..*"))

(defn cached-macro-ns? [name]
  (re-matches (cached-macro-ns-regexp) (str name)))

(defn cached-ns-regexp []
  (:clojure_cached_ns_regexp (klipse-settings) #"cljs\.stacktrace|cljs\.core\.[async|match].*|cljs\.spec.*|clojure.math\.combinatorics|clojure.test.check.*|cljs\.test|clojure\.test|reagent\..*|om\..*|clojure\.data|cljs-time\..*|re-frame\..*|day8\.re-frame\..*|reagent-forms\..*|re-frisk\..*|re-frisk-shell\..*|datafrisk\..*|ajax\..*|cljs\.tools\.reader\..*"))
...
(defn bundled-ns?
  ...
  (if (or (buggy-bundled-ns-ignore? name)
    ...
    false
    (or (!> js/goog.getObjectByName (str (munge name)))
        (!> js/goog.getObjectByName (string/replace (str (munge name)) #"clojure" "cljs"))))) ; (:require goog breaks the build see http://dev.clojure.org/jira/browse/CLJS-1677
...
(defn rename-ns [name]
  (let [name (str name)]
    (if (re-matches #"(?:clojure|cljs)\.spec\.impl\.gen" name)
      "clojure.spec.gen.alpha"
      (if (and
           (not (re-matches #".*alpha.*" name))
           (re-matches #"(?:clojure|cljs)\.spec.*" name))
        (str name ".alpha")
        name))))
      ...
      (bundled-ns? name) (let [_ (when (verbose?) (js/console.log "load-ns :cljs bundled" name))
                           ...
                           (go
                             (when-not (<! (try-to-load-ns filenames :js :cache src-cb :transform edn :can-recover? true))
                               ...
                               (src-cb {:lang :js :source ""}))))
      (cached-cljs-ns? name) (do
                               (when (verbose?) (js/console.info "load-ns :cljs cached cljs ns" (str name)))
                               (load-ns-from-cache name src-cb false))
      (cljsjs? name) (do
                       (when (verbose?) (js/console.info "load-ns :cljs cljsjs" (str name)))(try-to-load-cljsjs-ns name src-cb))
